...
Here, we study the genomic location of FP and FN calls with the genome stratification from GIAB [v3.4](https://ftp-trace.ncbi.nlm.nih.gov/ReferenceSamples/giab/release/genome-stratifications/v3.4/). This analysis is limited on chromosome 1-22.
...
For SNV, I would like to check if there are clusters of specific calls in certain region. First visualise in a raindrop plot, then separate them by chromosomes. The `cytoBand.txt.gz` was download from [ucsc ftp sever](https://hgdownload.soe.ucsc.edu/goldenPath/hg38/database/)

### COLO829 using ClairS 

```{r, warning=FALSE, fig.height=6, fig.width=6.5}
