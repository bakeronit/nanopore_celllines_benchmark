...
somatic_benchmark_path <- paste0(revision_workdir, "analysis/benchmark/purity/snvs")
...
df |> group_by(tool, celltype, Type) |> summarise(ave_precision=mean(Precision)*100, ave_recall=mean(Recall)*100) |>  knitr::kable()
```

```{r, warning=FALSE, fig.height=3.8, fig.width=6.2}
df |> 
...
```
The ROC curves show performance of snv calling under difference qual scores.

```{r, warning=FALSE}
roc_df <- 
  #list.files(paste0(workdir, "analysis/benchmark/snvs/somatic/R10/sup"), pattern = "^roc$", recursive = TRUE, full.names = TRUE) |> 
  list.files(paste0(revision_workdir, "analysis/benchmark/purity/snvs"), pattern = "^roc$", recursive = TRUE, full.names = TRUE) |> 
  discard(\(x) grepl("noXY",x)) |> 
  set_names(\(x) str_remove(x, revision_workdir) ) |> 
  ...
  mutate(tool=ifelse(grepl("clairS",prefix),"ClairS","DeepSomatic"), 
         sample=str_split_i(prefix,"/",i=6) |> str_split_i("\\.",i=1), 
         normal=str_split_i(prefix,"/",i=6) |> str_split_i("\\.",i=2), 
...
And we are wondering if the best QUAL threshold changed for samples with different tumour purities. Apparently, ClairS need a higher QUAL threshold than DeepSomatic.
...
**Figure 3: The QUAL thresholds which maximise the F1 score for samples with different tumour purity**
...
**Figure 4: SNV calling recall and precision under varying tumour purity levels in COLO829 and HCC1937 using ClairS and DeepSomatic**

```{r,fig.height=3.2, fig.width=6.2, eval=FALSE, warning=FALSE}
...
**Figure 5: VAF, DP and QUAL score distribution of SNV true positive and false positive calls in COLO829 and HCC1937**
...
**Figure 6: VAF distribution in ridgeline plot of SNV true positive and false positive calls in COLO829 and HCC1937**
...
somatic_benchmark_indel_path <- paste0(revision_workdir, "analysis/benchmark/purity/indels")
...
df_indel |> filter(Type=="INDEL") |> group_by(tool, celltype) |> summarise(ave_precision=mean(Precision)*100, ave_recall=mean(Recall)*100) |> knitr::kable()
```

```{r,  warning=FALSE, fig.height=3.8, fig.width=6.2}
df_indel |> 
  mutate(tool=ifelse(tool=="clairS","ClairS","DeepSomatic")) |> filter(Type=="INDEL") |> 
  ggplot(aes(x=Recall,y=Precision,group=tool,fill=purity,shape=tool)) +
  geom_path(color="darkgrey", linewidth=.4) +
  geom_point(size=2.5, color="black", alpha=.8) +
  scale_shape_manual(values=c(21,24)) +
  ...
  theme(legend.key.size = unit(0, "cm"), 
        legend.position = "bottom") +
  labs(shape="", fill="Purity") 
ggsave("figures/indel_calling_benchmark_precision_recall_curve_plot.png", width = 6.2, height = 3.8)
```
...
**Figure 7: Indel calling recall and precision under varying tumour purity levels in COLO829 and HCC1937 using ClairS and DeepSomatic**
...
**Figure 8: VAF, DP and QUAL score distribution of indel true positive and false positive calls in COLO829 and HCC1937**
