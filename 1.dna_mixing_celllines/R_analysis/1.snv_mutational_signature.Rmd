...
```{r}
read_tsv("../../gs/vcfs/colo829/merged_normed_isec_snv.hom100.goldstandard.vcf.gz", col_names = FALSE, comment = "#") |> 
...
read_tsv("../../gs/vcfs/hcc1937/merged_normed_isec_indel.goldstandard.vcf.gz", col_names = FALSE, comment = "#") |> 
  dplyr::select(X1, X2, X4, X5) |> dplyr::rename("chr"=X1,"pos"=X2, "ref"=X4, "alt"=X5) |> add_column(sample="hcc1937") |> filter(chr %in% chrom_chr) |> 
  mutate(chr=factor(chr, levels=chrom_chr)) |> 
  ggplot(aes(x=chr)) + 
  geom_bar()
...
```{r}
clairS_lr <- list.files("../../1.dna_mixing_celllines/work/analysis/snvs/clairS/R10/sup", pattern = "output.vcf.gz$", full.names = TRUE, recursive = TRUE) |> 
...
dpsomatic_lr <- list.files("../../1.dna_mixing_celllines/work/analysis/snvs/deepsomatic//R10/sup", pattern = "output.somatic.vcf.gz$", full.names = TRUE, recursive = TRUE) |> 
  set_names(\(f) dirname(f)) |> 
  map(\(f) read_tsv(f, col_names=FALSE,comment = "#") |> dplyr::select(X1, X2, X4, X5, X6) |> dplyr::rename("chr"=X1,"pos"=X2, "ref"=X4, "alt"=X5, "qual"=X6)  ) |> 
  list_rbind(names_to = "prefix") |> 
  mutate(sample=basename(prefix) |> str_split_i("\\.",i=1), tool=str_extract(prefix,"deepsomatic|clairS"), purity=str_split_i(sample,"_",i=2), purity=ifelse(is.na(purity),"100",purity))|> dplyr::select(-prefix)
...
gc_colo829 <- read_tsv("../../gs/vcfs/colo829/merged_normed_isec_snv.hom100.goldstandard.vcf.gz", col_names = FALSE, comment = "#") |> 
...
cosmicv2_h38 <- read_table("data/COSMIC_v2_SBS_GRCh38.txt") |> column_to_rownames("Type")
...
```{r,fig.height=2.6, fig.width=10.6}
gc_hcc1937 <- read_tsv("../../gs/vcfs/hcc1937/merged_normed_isec_snv.hom100.goldstandard.vcf.gz", col_names = FALSE, comment = "#") |> 
