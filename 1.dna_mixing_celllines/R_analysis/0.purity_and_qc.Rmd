...
df_mosdepth |> separate_wider_delim(sample,"_", names = c("celltype","purity"), too_few = "align_start") |> 
  group_by(celltype) |> summarise(mean_depth=mean(mean), max_depth=max(mean), min_depth=min(mean)) |> knitr::kable()
```
...
df <- list.files("../0.purity_check/dna_mixing/vcf_files/deepsomatic/cn2/", pattern = "vcf$", full.names = TRUE) |> 
