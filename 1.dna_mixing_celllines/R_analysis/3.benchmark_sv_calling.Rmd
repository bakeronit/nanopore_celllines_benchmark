...
### The benchmarking of SV calling

We used Jasmine to preprocess LR SVs and to merge them with short-read gold standard SV for each cell line as described in [benchmark_sv.smk](../nanopore_paired_tumour_workflow/workflow/benchmark_sv.smk).
...
Try to visualise with facet_nested
...
Try to visualise without facet_nested
...
**Precision and recall rate of SV calling**
...
I think combining SVs detected by multiple tools is a good strategy to call as many novel SV as possibly while keep confidence. Here I compared the the intersect of four tools and pick any SVs that are detected by at least two tools with short-read 'gold standard'.
...
```{r, warning=FALSE, fig.height=3.8, fig.width=6.4}
temp_df_sv |> 
  ggplot(aes(x=recall,y=precision, group=tool, fill=purity, color=tool)) +
  geom_path( linewidth=1, alpha=.6) +
  geom_point(size=1.2, color="black", alpha=.8, shape=22) +
  ggsci::scale_color_aaas(labels=public_tool_names) +
  scale_fill_manual(values=purity_pals) +
  facet_wrap(~cellline) + ylim(0,1) + xlim(0,1) + 
  theme_bw(base_size = 12)  + 
  theme(legend.key.size = unit(.2, "cm"), 
        legend.position = "bottom") +
  labs(x="Recall", y="Precision", fill="Purity", color="") 
ggsave("figures/sv_calling_benchmark_precision_recall_curve_plot.png", width = 6.4, height = 3.8)
```
