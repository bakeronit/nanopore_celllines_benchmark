"""
Perform the same benchmark but with non-standard chromosome excluded as I noticed that deepsomatic didn't generate any.
"""
include: "helper.smk"
...
rule all:
    input:
        [f"analysis/benchmark/purity/snvs/clairS/{pair['tumour']}.{pair['normal']}/summary.txt" for pair in pairs],
        [f"analysis/benchmark/purity/indels/clairS/{pair['tumour']}.{pair['normal']}/summary.txt" for pair in pairs],
        [f"analysis/benchmark/purity/snvs/deepsomatic/{pair['tumour']}.{pair['normal']}/summary.txt" for pair in pairs],
        [f"analysis/benchmark/purity/indels/deepsomatic/{pair['tumour']}.{pair['normal']}/summary.txt" for pair in pairs]
...
def get_vcf(wildcards):
    ...
    vcf_files = {
        'deepsomatic': {
            'snvs': purity_workdir / 'analysis/snvs/deepsomatic/R10/sup/{sample_t}.{sample_n}/output.somatic.vcf.gz',
            'indels': purity_workdir / 'analysis/snvs/deepsomatic/R10/sup/{sample_t}.{sample_n}/norm_indel.vcf.gz'
        },
        'clairS': {
            'snvs': purity_workdir / 'analysis/snvs/clairS/R10/sup/{sample_t}.{sample_n}/output.vcf.gz',
            'indels': purity_workdir / 'analysis/snvs/clairS/R10/sup/{sample_t}.{sample_n}/norm_indel.vcf.gz'
        }
    }
...
use rule clean_goldstandard as clean_vcf with:
    input:
        get_vcf
    output:
        temp("analysis/benchmark/purity/{variant_type}/{tool}/{sample_t}.{sample_n}.clean_chrom.vcf.gz")

ClairS_sif = config['clairS']['sif']
rule benchmark_vcf:
    input:
        vcf =  "analysis/benchmark/purity/{variant_type}/{tool}/{sample_t}.{sample_n}.clean_chrom.vcf.gz",
    ...
    output:
        "analysis/benchmark/purity/{variant_type}/{tool}/{sample_t}.{sample_n}/summary.txt"
    params:
        outdir = "analysis/benchmark/purity/{variant_type}/{tool}/{sample_t}.{sample_n}"
    threads: 4
    resources:
        mem = 20,
        walltime = 2
